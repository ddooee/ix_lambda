...
if (CLIENT) then
	...
	function ITEM:PopulateTooltip(tooltip)
		...
		if self.hasLock then
			...
			lock:SetText("미허가된 생명체의 화기 무단 사용에 대한 보호 프로토콜이 작동중입니다.")
		...
		if self.Info then
			if self.Info.Type then
				...
				skill:SetText(string.format("유형: %s", Types[self.Info.Type]))
				skill:SetBackgroundColor(grayClr)
				skill:SizeToContents()
			end

			local skill = tooltip:AddRow("skill")
			skill:SetText(string.format("속성: %s", string.utf8lower(L((ix.skills.list[self.Info.Skill] or {}).name) or "알 수 없음")))
			skill:SetBackgroundColor(grayClr)
			skill:SizeToContents()

			if self.Info.Distance then
				local penalty = tooltip:AddRow("penalty")
				penalty:SetText("거리 보너스:")
			...
			if self.Info.Dmg then
				...
				if self.Info.Dmg.Attack then
					...
					AP:SetIcon(icons[2])
					AP:SetText(string.format("%s 공격", self.Info.Dmg.Attack))
					AP:SizeToContents()
				end
				...
				if self.Info.Dmg.AP then
					local AP = AddIconRow(tooltip)
					AP:SetIcon(icons[1])
					AP:SetText(string.format("%s 관통률", self.Info.Dmg.AP))
					AP:SizeToContents()
				end
				...
				if self.Info.Dmg.Limb then
					...
					DMG:SetIcon(icons[3])
					DMG:SetText(string.format("%s 피해", self.Info.Dmg.Limb))
					DMG:SizeToContents()
				end
				...
				if self.Info.Dmg.Shock then
					local DMG = AddIconRow(tooltip)
					DMG:SetIcon(icons[4])
					DMG:SetText(string.format("%s — %s 충격", self.Info.Dmg.Shock[1], self.Info.Dmg.Shock[2]))
					DMG:SizeToContents()
				end
				...
				if self.Info.Dmg.Blood then
					local DMG = AddIconRow(tooltip)
					DMG:SetIcon(icons[5])
					DMG:SetText(string.format("%s — %s 혈액", self.Info.Dmg.Blood[1], self.Info.Dmg.Blood[2]))
					DMG:SizeToContents()
				end
				...
				if self.Info.Dmg.Bleed then
					...
					Bleed:SetText(string.format("%s%%의 출혈 기회", self.Info.Dmg.Bleed))
...
ITEM.functions.awFix = {
	...
	OnRun = function(item)
		...
		client:SetAction("수리중...",repairtime,function()

			client:Notify("성공적으로 화기를 수리했습니다.")
