...
function PANEL:Init()
	...
	self.icon.DoRightClick = function()
		...
		menu:AddOption("Скопировать Anon ID", function()
			SetClipboardText(client:IsBot() and client:EntIndex() or client:GetAnonID())
		end)

		if ix.plugin.list["!!!anonymous"].deanon then
			menu:AddOption("Скопировать Steam ID", function()
				SetClipboardText(client:IsBot() and client:EntIndex() or client:AnonSteamID())
			end)
		end
...
local donatorClr = Color(100, 200, 255)
function PANEL:Update()
	...
	name = bRecognize and name or L"unknown"
...
function PANEL:SetFaction(faction, scoreboardClass)
	if scoreboardClass and isbool(faction) then
		faction = {}

		for i = 1, #ix.faction.indices do
			local data = ix.faction.indices[i]

			if data.scoreboardClass == scoreboardClass then
				faction[#faction+1] = data.index
			end
		end
	end
	...
	if istable(faction) then
		self:SetColor(Schema.scoreboardClasses[scoreboardClass] or color_white)
		self:SetText(L(scoreboardClass))
	else
...
function PANEL:Update()
	...
	if !istable(faction) then
		faction = ix.faction.indices[faction]
		...
		for k, f in pairs(faction) do
			for k, v in ipairs(team.GetPlayers(f)) do
				if !IsValid(v.ixScoreboardSlot) then
					self:AddPlayer(v, iHasPlayers)
				else
					v.ixScoreboardSlot:Update()
				end

				iHasPlayers = iHasPlayers + 1
			end
		end

		self:SetVisible(iHasPlayers > 0)
		if iHasPlayers == 0 then
			self:GetParent():InvalidateLayout()
		end
	end
end
...
function PANEL:Init()
	...
	for i = 1, #ix.faction.indices do
		...
		if faction.scoreboardClass then
			if !available[faction.scoreboardClass] then
				available[faction.scoreboardClass] = {L(faction.scoreboardClass), true}
			end

			continue
		end

		available[faction.uniqueID] = {faction.name, faction.index}
	end

	for i, data in SortedPairsByMemberValue(available, 1, true) do
		local panel = self:Add("ixScoreboardFaction")
		panel:SetFaction(data[2], i)
		panel:Dock(TOP)

		self.factions[#self.factions + 1] = panel
	end
