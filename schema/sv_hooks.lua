...
function Schema:PlayerFootstep(client, position, foot, soundName, volume)
	...
	if (client:GetCharacter():GetData("heavy") and client:IsRunning()) then
		client:EmitSound(({[0] = "NPC_CombineS.RunFootstepLeft", [1] = "NPC_CombineS.RunFootstepRight"})[foot])
		--FACTION.runSounds = {[0] = "NPC_CombineS.RunFootstepLeft", [1] = "NPC_CombineS.RunFootstepRight"}
		return true
	end
	
	if (client:GetCharacter():GetData("heavy") and client:IsRunning() and client:Team() == FACTION_MPF) then
...
function Schema:PlayerDeath(client, inflicter, attacker)
	if (client:IsCombine()) then
		local letter = dispatch.AddWaypoint(client:GetShootPos(), "생체 신호 손실", "death", 60)
		self:AddCombineDisplayMessage(string.format("병력 %s: 생체신호가 손실됨!", letter), color_red)
...
function Schema:GetPlayerDeathSound(client)
	if (client:IsCombine()) then
		local sound = "awhl2rp/metrocop/fire_scream" .. math.random(2, 3) .. ".wav"
