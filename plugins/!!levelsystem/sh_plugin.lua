...
ix.chat.Register("level", {
	...
	OnChatAdd = function(self, speaker, text, bAnonymous, data)
		if data.t == 1 then
			chat.AddText(color_white, string.format("%s로 업그레이드되었습니다! 새 포인트를 할당하려면 /lvl을 입력하십시오.", LocalPlayer():GetCharacter():GetLevel()))
		elseif data.t == 2 then
			local name = L((ix.skills.list[data.skill] or {}).name) or "Unknown"

			chat.AddText(color_white, string.format("%s 스킬이 %s로 향상되었습니다!", name, data.value))
		elseif data.t == 3 then
			chat.AddText(color_white, string.format("레벨이 %s로 떨어졌습니다!", LocalPlayer():GetCharacter():GetLevel()))
		elseif data.t == 4 then
			local name = L((ix.skills.list[data.skill] or {}).name) or "Unknown"

			chat.AddText(color_white, string.format("%s 스킬이%s로 떨어졌습니다!", name, data.value))
		end
	end
})
