...
function PANEL:Init()
	if IsValid(cellar_tab_scoreboard) then
		cellar_tab_scoreboard:Remove()
	end

	cellar_tab_scoreboard = self
    ...
    local scoreboard = self.canvas:Add('ixScoreboard')
...
vgui.Register("cellar.tab.scoreboard", PANEL, "EditablePanel")

if IsValid(cellar_tab_scoreboard) then
	cellar_tab_scoreboard:Remove()
	cellar_tab_scoreboard = nil
end