...
function PANEL:Init()
	if IsValid(cellar_tab_config) then
		cellar_tab_config:Remove()
	end

	cellar_tab_config = self
    ...
    local config = self.canvas:Add('ixConfigManager')
...
vgui.Register("cellar.tab.config", PANEL, "EditablePanel")

if IsValid(cellar_tab_config) then
	cellar_tab_config:Remove()
	cellar_tab_config = nil
end