...
function PANEL:Paint(w, h)
	...
	/*raw.SimpleText("COLONIAL SUPER-SOLDIERS", "cellar.tab", ScrW() - ScrW()/2 + 36, ScrH()/3.9 + 30, Color(43, 157, 189, 90 + tsin), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
	draw.SimpleText("COLONIAL SUPER-SOLDIERS", "cellar.tab.blur", ScrW() - ScrW()/2 + 36, ScrH()/3.9 + 30, Color(43, 157, 189,  90 + tsin), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)*/

	surface.SetDrawColor(ColorAlpha(color_white, 90 + tsin))
	surface.SetMaterial(yufulogo)
	surface.DrawTexturedRect(ScrW() - 256 - 12, ScrH()/2.75, 256, 64)

	draw.SimpleText("베타 버전: 일부 기능이 누락되거나 제대로 작동하지 않을 수 있습니다.", "cellar.tab",  ScrW() - ScrW()/2, ScrH() - ScrH()/2.4, ColorAlpha(color_orange,  90 + tsin), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
	draw.SimpleText("베타 버전: 일부 기능이 누락되거나 제대로 작동하지 않을 수 있습니다.", "cellar.tab.blur",  ScrW() - ScrW()/2, ScrH() - ScrH()/2.4, ColorAlpha(color_orange,  90 + tsin), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)



	draw.SimpleText("사용자 인터페이스의 디자인 및 기능은", "cellar.watermark", ScrW() - ScrW()/2, ScrH() - ScrH()/1.7, color_white, TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
	draw.SimpleText("Cellar Team에서 작성, 설계 및 제공되었습니다.", "cellar.watermark", ScrW() - ScrW()/2, ScrH() - ScrH()/1.7 + 24, color_white, TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
	...
	draw.SimpleText("신인류 감시부대 역할극", "cellar.tab", 10, ScrH() - ScrH()/3, ColorAlpha(color_red, 7), TEXT_ALIGN_LEFT, TEXT_ALIGN_CENTER)
	draw.SimpleText("신인류 감시부대 역할극", "cellar.tab.blur", 10, ScrH() - ScrH()/3, ColorAlpha(color_red, 7), TEXT_ALIGN_LEFT, TEXT_ALIGN_CENTER)
	draw.SimpleText("GUI BY CELLAR TEAM", "cellar.tab", ScrW() - 10, ScrH() - ScrH()/3, ColorAlpha(color_red, 7), TEXT_ALIGN_RIGHT, TEXT_ALIGN_CENTER)
	draw.SimpleText("GUI BY CELLAR TEAM", "cellar.tab.blur", ScrW() - 10, ScrH() - ScrH()/3, ColorAlpha(color_red, 7), TEXT_ALIGN_RIGHT, TEXT_ALIGN_CENTER)
	draw.SimpleText("2023. ALL RIGHTS RESERVED", "cellar.tab", ScrW() - ScrW()/2, ScrH() - ScrH()/3, ColorAlpha(color_red, 7), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
	draw.SimpleText("2023. ALL RIGHTS RESERVED", "cellar.tab.blur", ScrW() - ScrW()/2, ScrH() - ScrH()/3, ColorAlpha(color_red, 7), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
