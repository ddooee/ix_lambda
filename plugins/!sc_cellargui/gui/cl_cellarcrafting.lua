...
function PANEL:Init()
	if IsValid(cellar_tab_crafting) then
		cellar_tab_crafting:Remove()
	end

	cellar_tab_crafting = self
    ...
    --self.canvas = self:Add("cellar.tab.frame")
    ...
    local crafting = self.canvas:Add('ixCrafting')
    crafting:Dock(FILL)
...
vgui.Register("cellar.tab.crafting", PANEL, "EditablePanel")

if IsValid(cellar_tab_crafting) then
	cellar_tab_crafting:Remove()
	cellar_tab_crafting = nil
end