...
if (SERVER) then
	...
	function ENT:Use( activator, caller )
		...
		if self:GetNetVar("reward_done") == true then
			...
			char:GetPlayer():NotifyLocalized("성공적으로 보상을 받았습니다.")
			...
			char:GetPlayer():NotifyLocalized("아직 보상을 받을 준비가 되지 않았습니다...")
		end
	end
	...
	function ENT:Draw()
		...
		if self:GetNetVar("nv_metro") == true then metro_control = "통제중" metro_col = Color( 0, 228, 34, 189) else metro_control = "통제권 없음" metro_col = Color( 214, 0, 0, 202) end
		if self:GetNetVar("nv_otabridge") == true then otabridge_control = "통제중" otabridge_col = Color( 0, 228, 34, 189) else otabridge_control = "통제권 없음" otabridge_col = Color( 214, 0, 0, 202) end
		if self:GetNetVar("nv_village") == true then village_control = "통제중" village_col = Color( 0, 228, 34, 189)	else village_control = "통제권 없음" village_col = Color( 214, 0, 0, 202) end
		if self:GetNetVar("nv_destroyedvillage") == true then destroyedvillage_control = "통제중" destroyedvillage_col = Color( 0, 228, 34, 189) else destroyedvillage_control = "통제권 없음" destroyedvillage_col = Color( 214, 0, 0, 202) end
		if self:GetNetVar("nv_canalspit") == true then canalspit_control = "통제중" canalspit_col = Color( 0, 228, 34, 189) else canalspit_control = "통제권 없음" canalspit_col = Color( 214, 0, 0, 202)	end
		if self:GetNetVar("nv_fisherhouse") == true then fisherhouse_control = "통제중" fisherhouse_col = Color( 0, 228, 34, 189) else fisherhouse_control = "통제권 없음" fisherhouse_col = Color( 214, 0, 0, 202) end
		if self:GetNetVar("nv_mines") == true then mines_control = "통제중" mines_col = Color( 0, 228, 34, 189) else mines_control = "통제권 없음" mines_col = Color( 214, 0, 0, 202)	end
		...
		cam.Start3D2D(fixedPos, fixedAng, 0.1)
			...
			draw.SimpleText( "보급완료까지:", "Default", 50, 0, Color( 255, 255, 255, 155 ), TEXT_ALIGN_CENTER)
		...
		cam.Start3D2D(fixedPos_title, fixedAng, 0.1)
			draw.SimpleText( "저항군", "Default", 50, 0, Color( 255, 255, 255, 155 ), TEXT_ALIGN_CENTER)
		cam.End3D2D()
		...
		cam.Start3D2D(fixedPos_metro, fixedAng, 0.1)
			draw.SimpleText( "기차역", "Default", 50, 0, Color( 255, 255, 255, 155 ), TEXT_ALIGN_CENTER)
		...
		cam.Start3D2D(fixedPos_otabridge, fixedAng, 0.1)
			draw.SimpleText( "넥서스", "Default", 50, 0, Color( 255, 255, 255, 155 ), TEXT_ALIGN_CENTER)
		...
		cam.Start3D2D(fixedPos_village, fixedAng, 0.1)
			draw.SimpleText( "도시", "Default", 50, 0, Color( 255, 255, 255, 155 ), TEXT_ALIGN_CENTER)
		...
		cam.Start3D2D(fixedPos_destroyedvillage, fixedAng, 0.1)
			draw.SimpleText( "웨이스트랜드", "Default", 50, 0, Color( 255, 255, 255, 155 ), TEXT_ALIGN_CENTER)
		...
		cam.Start3D2D(fixedPos_canalspit, fixedAng, 0.1)
			draw.SimpleText( "운하", "Default", 50, 0, Color( 255, 255, 255, 155 ), TEXT_ALIGN_CENTER)
		...
		cam.Start3D2D(fixedPos_fisherhouse, fixedAng, 0.1)
			draw.SimpleText( "해변가", "Default", 50, 0, Color( 255, 255, 255, 155 ), TEXT_ALIGN_CENTER)
		...
		cam.Start3D2D(fixedPos_mines, fixedAng, 0.1)
			draw.SimpleText( "광산", "Default", 50, 0, Color( 255, 255, 255, 155 ), TEXT_ALIGN_CENTER)
