...
do
	...
	ix.chat.Register("dmgMsg", {
		...
		OnChatAdd = function(self, speaker, text, bAnonymous, data)
			...
			elseif data.t == 2 then
				chat.AddText(color_white, "당신은 소지품의 30%를 잃었습니다.")
			elseif data.t == 3 then
				chat.AddText(color_white, "중단하였습니다.")
			end
		end
	})
	...
	ix.chat.Register("dmgAdminMsg", {
		...
		OnChatAdd = function(self, speaker, text, bAnonymous, data)
			...
			if data.t == 1 then
				chat.AddText(clrRed, string.format("플레이어%s (%s) 가 플레이어%s (%s) 를 사살하려고 합니다!", data.attacker:Name(), data.attacker:GetAnonID(), data.crit:Name(), data.crit:GetAnonID()))
			elseif data.t == 2 then
				chat.AddText(clrRed, string.format("%s (%s) 이(가) %s (%s) 를 사살했습니다!", data.crit:Name(), data.crit:GetAnonID(), data.attacker:Name(), data.attacker:GetAnonID()))
			end
		end
	})
end
