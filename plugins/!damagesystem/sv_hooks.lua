	...
	function PLAYER:SetCriticalState(state)
		...
		if state then
			...
			if self:IsCombine() then
				local letter = dispatch.AddWaypoint(self:GetShootPos(), "생체 신호 손실", "death", 30)
				Schema:AddCombineDisplayMessage(string.format("병력 %s: 생체 신호 손실!", letter), color_red)
			end
...
do
	...
	net.Receive("ixCritApply", function(len, client)
		...
		if state then
			...
			local character = client:GetCharacter()
			client:SetAction("당신은 캐릭터를 쫓고 있어요...", 15)
...
net.Receive("ixCritUse", function(len, client)
	...
	if client:GetCharacter():GetLevel() < 3 then
		client:Notify("그 행동을 하기에는 아직 부족합니다!")
		return
	end
	...
	if admins <= 0 then
		client:Notify("서버에 관리자가 존재하지 않습니다.")
		return
	end
