...
if (CLIENT) then
		...
		if (LocalPlayer():GetChar()) then
			for k, v in pairs(ents.FindByClass("prop_ragdoll")) do
				if IsValid(v) and v.isDeadBody then
					...
					draw.SimpleText("남은 시간: "..math.Round(v:GetNWFloat("Time") - CurTime()), "ReviveText", Pos.x, Pos.y - 20, Color(249, 255, 239), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
				end
			end
		end
	end)
  ...
  function PLUGIN:DoPlayerDeath( client, attacker, dmg )
		...
		client:Notify(""..math.Round(ix.config.Get("spawnTime", 10)).."초 후 리스폰됩니다.")
...
ix.command.Add("charrevive", {
    ...
    OnRun = function(self, client, target, health)
        ...
        if target ~= client then
            client:Notify("당신은 " .. target:GetName() .. "을(를) 소생시켰습니다.")
            target:Notify("당신은 " .. client:GetName() .. "에 의해 소생되었습니다.")
        else
            if target == client then
                client:Notify("당신은 스스로를 소생시켰습니다.")
            end
        end
    end
})
