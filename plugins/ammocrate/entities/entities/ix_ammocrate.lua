...
if ( SERVER ) then
    ...
    function ENT:Use(ply)
        if ( IsValid(self) ) then
            ...
            if not ( ammo[game.GetAmmoName(ply:GetActiveWeapon():GetPrimaryAmmoType())] ) then
                ply:Notify("올바른 무기를 든 채로 사용하여야 합니다.")
                return
            else
                net.Start("OpenCrate")
    ...
    net.Receive("OpenCrate", function(len)
        ...
        Derma_StringRequest("Ammunition", "가져갈 탄약 수", s.max, function(amount)
            amount = tonumber(amount)
            if ( isnumber(amount) ) then 
                if ( amount > tonumber(s.max) ) then
                    ply:Notify("당신은 "..s.max.."만큼의 탄약을 가져갈 수 없습니다.")
                    return
                end
