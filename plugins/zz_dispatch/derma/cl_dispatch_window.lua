...
do
	...
	function PANEL:Init()
		...
		do
			...
			self.manpower:SetWindowName("구성")
			self.manpower.stats = {}
	...
	function PANEL:SwitchStability() 
		...
		for k, v in ipairs(dispatch.stability_codes) do
			menu:AddOption(v.name, function()
				Derma_Query(string.format("현재 사회안정도 코드를 %s로 바꾸시겠습니까?", v.name), "안정도 변경", "적용", function()
