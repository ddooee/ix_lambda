...
function PANEL:Init()
	...
	self.hp:SetText("100%")
	self.code_text:SetText("코드 1")
	self.area_text:SetText("N/A")
...
function PANEL:OpenMenu(target)
	...
	if isDispatch then
		...
		for k, v in pairs(dispatch.GetSquads()) do
			...
			move:AddOption(k == 1 and "분대 해산" or v:GetTagName(), function()
		...
		if isLeader then
			squad:AddOption("분대 추방", function()
				net.Start("squad.menu.move")
					net.WriteUInt(id, 32)
					net.WriteBool(true)
				net.SendToServer()
	...
	if isDispatch then
		menu:AddOption("감시", function() 
			net.Start("squad.menu.spectate")
				net.WriteUInt(id, 32)
			net.SendToServer()
		end):SetImage("icon16/zoom.png")

		menu:AddOption("살균 점수 부여", function() 
			Derma_StringRequest(
				target:GetName(), 
				"발급할 살균 점수를 입력하십시오 (+ 혹은 -)",
				"3",
				function(value) 
	...
	menu:AddSpacer()
	menu:AddOption("개인 파일 열기", function() 
