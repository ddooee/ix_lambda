...
dispatch.WorldAction({
	...
	Receive = function(self, client)
		...
		local letter = dispatch.AddWaypoint(entity:GetShootPos(), "BOL 대상자"..data, "warn", 60)

		Schema:AddCombineDisplayMessage(string.format("태그 %s: %s", letter, card and ("시민"..data.." 은(는) BOL 대상자로 지정됨.") or "신원 미상의 시민이 BOL 대상자로 지정됨!"), color_red)
	end
})
...
dispatch.WorldAction({
	...
	Action = function(self, entity, client, trace)
		...
		m:AddOption("시민 통행 가능", function()
			self:MsgStart()
				net.WriteEntity(entity)
				net.WriteUInt(0, 3)
			self:MsgEnd()
		end)
		m:AddOption("통행 제한", function()
			self:MsgStart()
				net.WriteEntity(entity)
				net.WriteUInt(1, 3)
			self:MsgEnd()
		end)
		m:AddOption("접근 금지", function()
			self:MsgStart()
				net.WriteEntity(entity)
				net.WriteUInt(2, 3)
			self:MsgEnd()
