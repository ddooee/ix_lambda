...
function ENT:OnDeath(camera, client)
	local letter = dispatch.AddWaypoint(camera:GetPos(), "경고: 카메라의 신호가 소실됨", "warn", 60)
		
	Schema:AddCombineDisplayMessage(string.format("대상 %s: 신호가 소실됨!", letter), color_red)
...
function ENT:ShouldHate(client)
	if client:IsNPC() then
		local alarm, alarmtext, alarmtext2 = false, "대상 %s: ", ""

		if client:Classify() == CLASS_HEADCRAB then
			alarmtext = alarmtext.."기생충에 의해 감염됨!"
			alarmtext2 = "기생충"
			alarm = true
		elseif client:Classify() == CLASS_ANTLION then
			alarmtext = alarmtext.."엑소젠의 침입이 감지됨!"
			alarmtext2 = "엑소젠"
			alarm = true
		elseif client:Classify() == CLASS_ZOMBIE then
			alarmtext = alarmtext.."네크로틱의 침입이 감지됨!"
			alarmtext2 = "네크로틱"
			alarm = true
		end
	...
	if client:Team() == FACTION_VORTIGAUNT then
		if client:HasWeapon("ix_vortbeam") then
			return true, "대상 %s: 구속되지 않은 바이오틱 포착됨!", "구속되지 않은 바이오틱", "hazard", color_red
		end
	...
	elseif !client:IsCombine() and !client:IsCityAdmin() then
		...
		for _, weapon in pairs(client:GetWeapons()) do
			if IsValid(weapon) and weapon.ixItem then
				return true, "대상 %s: "..(card and "무장중인" or "미확인된 화기를 소지중인").." 647E! "..textData, "94B 647E "..textData, "gun", color_red
			end
		end
		...
		if card then
			...
			return true, "대상 %s: 신원 미상의 시민이 감지됨!", "신원 미상의 시민", "poi", yellow
		end

		if datafile and data then
			if data.status == "Anti-Citizen" then
				return true, "대상 %s: 반-시민이 포착됨! "..textData, "반-시민"..textData, "warn", color_red
			elseif data.bol then
				return true, "대상 %s: 감시중"..textData, textData, "poi", yellow
			end
		end
	end
