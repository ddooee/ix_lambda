...
if (SERVER) then
	...
	function ENT:DissolvePlayer(pl, pos)
		...
		self:EmitSound("awhl2rp/walloflight/wolspark.wav")
...
return entity end
	...
	function ENT:Think()
		...
		if not self.sound_loop then
			self.sound_loop = CreateSound(self, 'awhl2rp/combine_terminals/quarantine_forcefield_lp_01.mp3')
		...
		if self:GetToggle() and self:GetNetVar("light") then
			if (not self.Beep or CurTime() >= self.Beep) then
				self:EmitSound("awhl2rp/walloflight/wol1.wav")
				self:GetDummy():EmitSound("awhl2rp/walloflight/wol1.wav")
				self.Beep = CurTime() + 2
			end
		end
						...
						if not v:GetCharacter():HasIDAccess(self:GetAccess()) and not v.ixObsData then
							...
							if not self:GetNetVar("light") then
								if (not self.Warning or CurTime() >= self.Warning) then
									self:EmitSound("awhl2rp/alarms/combine_alarm_oneshot.wav")
									self:GetDummy():EmitSound("awhl2rp/alarms/combine_alarm_oneshot.wav")
				...
				if self:GetNetVar("light") then
					...
					if self.BeepDouble then
						if (not self.Beep2 or CurTime() >= self.Beep2) then
							self:EmitSound("awhl2rp/walloflight/wol2.wav")
							self:GetDummy():EmitSound("awhl2rp/walloflight/wol2.wav")
	...
	function ENT:toggle(boolean)
		...
		if self:GetToggle() then
			...
			self:EmitSound('awhl2rp/combine_terminals/forcefield_citizen_start_01.mp3')
			self:GetDummy():EmitSound('awhl2rp/combine_terminals/forcefield_citizen_start_01.mp3')
		else
			self:SetSkin(1)
			self.dummy:SetSkin(1)
			self:EmitSound('awhl2rp/combine_terminals/forcefield_citizen_start_01.mp3')
			self:GetDummy():EmitSound('awhl2rp/combine_terminals/forcefield_citizen_stop_01.mp3')
		end
		--[[-------------------------------------------------------------------------
	...
	function ENT:Draw()
		...
		if self:GetToggle() then
			...
			if self.alpha > 0 then
				draw.SimpleText("미허가된 생체 신호 감지", "cp_hud4", (self:GetPos():Distance(dummy:GetPos()) * 16.5) / 2, 2120, Color(255, 255, 255, self.alpha), TEXT_ALIGN_CENTER, 1)
			end

			draw.SimpleText("제한구역", "cp_hud4", (self:GetPos():Distance(dummy:GetPos()) * 16.5) / 2, 1550, Color(255, 255, 255, math.abs(math.sin(CurTime())) * 200), TEXT_ALIGN_CENTER, 1)
			draw.SimpleText("유효한 생체 신호 필요", "cp_hud4", (self:GetPos():Distance(dummy:GetPos()) * 16.5) / 2, 1750, Color(255, 255, 255, math.random(150, 255)), TEXT_ALIGN_CENTER, 1)
			cam.End3D2D()
