-- "gamemodes\\ixhl2rp\\plugins\\rebeldeployables\\entities\\weapons\\weapon_conbarricade_placer.lua"
...
SWEP.PrintName = "Concrete Barricade Placer"
SWEP.Author = "Robert"
SWEP.Purpose = ""
...
SWEP.Ent = "ix_concretebarricade"
SWEP.BarricadePlace = nil
SWEP.PreviewModel = "models/props_c17/concrete_barrier001a.mdl"
	...
	if (SERVER) then
		...
		if (tr.HitWorld) then
				if (!self.BarricadePlace) then
					self.BarricadePlace = self.Owner:GetPos() + Angle(0, self.Owner:GetAngles().y, 0):Forward() * 65 + Angle(0, self.Owner:GetAngles().y, 0):Right() * -10
					self.testowyang = Angle(0, self.Owner:GetAngles().y, 0)
				end
...
function SWEP:SecondaryAttack()
	if (SERVER) then
		...
		inventory:Add("conbarricade_placer")
...
if (CLIENT) then
	function SWEP:DrawHUD()
		...
		draw.SimpleTextOutlined("좌클릭으로 설치", "CHudLabel", ScrW() / 2, ScrH()-230, Color(250,250,250), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER, 1, Color(25, 25, 25, 250))
		draw.SimpleTextOutlined("우클릭으로 취소", "CHudLabel", ScrW() / 2, ScrH()-200, Color(250,250,250), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER, 1, Color(25, 25, 25, 250))
	end
end
